...
Spelloopen i Client hanterar genom input() alla knapptryckningar och anropar asdasd i Controller som hanterar fallet då skjutknappen är nedtryckt. Knappen var nedtryckt och Controller säger åt sin Player att skjuta. Player säger åt Weapon att det skall avfyras och av vilket clientID. Vapnet minskar då sin ammunition och skapar ett Shot med clientID, startposition, riktningsvektor, och möjligheten att rita ut sig själv eftersom det är en sf::Drawable. Skottet returneras från vapnets fire() i en vektor till spelaren och vidare från spelaren till Controller och vidare därifrån till input() metoden som då byter ut unhandledShots i GameState mot denna vektor.
